...
if (getFyersToken() != None):
    # print("fyers inside if", getFyersToken())
    global fyers
...
def login():
    ...
    print("Session", session.set_token(auth_code))
    response = session.generate_token()['access_token']
...
def getLastTransactionByScriptId(id):
    try:
        ...
        values = (id)
        cursor.execute(sql, values)
        print("inside get Last Transaction", cursor.fetchone())
...
# @app.route('/getFyersFund', methods=['GET'])
# @token_required
# def get_fyers_fund():
#     # return jsonify(getFyersFund()['fund_limit'][0])
#     return jsonify(getFyersFund()['fund_limit'][0]['equityAmount'])

@app.route('/getFundSummary', methods=['GET'])
@token_required
def getFundSummary():
    fyersFund = getFyersFund()['fund_limit'][0]['equityAmount']
    try:
        sql = '{CALL getFundSummary()}'
        cursor.execute(sql)
        result = cursor.fetchone()
        return jsonify({'fyersFund': fyersFund, 'invValue': result[0], 'marketValue': result[1]})
    except:
        with open('errorLogs.txt', 'a+') as f:
            f.writelines(f"{'='*10}{datetime.now()}{'='*10}")
            traceback.print_exc(file=f)
...
# +++++++++++++++++++++++++++++++++Changes for correcting investment balance++++++++++++++++++++++++++++++++
                        investment_balance += qty_slab*last_transaction
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        last_traded_value = current_price
                        print(
                        ...
                        print(
                            "============================================================================================")
                        message = f"Sold at - {current_price} {qty_slab} qty at sm2 rate"
                        if stock_quantity == 0:
                            average_price = 0
                        else:
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                        ...
                        print(
                            "============================================================================================")
                        message = f"Sold at - {current_price} {qty_slab} qty"
                        if stock_quantity == 0:
                            average_price = 0
                        else:
                            average_price = (investment_fund -
